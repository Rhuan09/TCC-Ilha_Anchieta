...
public class LocationToShaderINFINIGRASS : MonoBehaviour {
	...
	void LateUpdate () {
        ...
        for (int i = 0; i < Materials.Count; i++)
        {
            ...
            if (enableCirclularWind)//SpeedVec.magnitude == 0)
            {
                Materials[i].SetVector("_InteractPos1", helicopter.position);
            }
            if (Materials[i].HasProperty("_InteractSpeed"))
            {
                //Grassmanager.GrassMaterials[i].SetVector("_InteractSpeed",SpeedVec);
                Materials[i].SetVector("_InteractSpeed", Vector3.Lerp(Materials[i].GetVector("_InteractSpeed"), -SpeedVec, InteractSpeed * Time.deltaTime));
               // Materials[i].SetVector("_InteractSpeed", Vector3.ClampMagnitude(Vector3.Lerp(Materials[i].GetVector("_InteractSpeed"), -SpeedVec, InteractSpeed * Time.deltaTime),2 ));
            }
        }
